...
import { getLoDatabyUser, updateLoDatabyUser} from '../../../../redux/actions/coc/loAction'
...
class Tabs extends React.Component {
  ...
  componentDidUpdate (prevProps, nextProps) {
    if (prevProps.values !== this.props.values)
      this.setState({ capturedLOData: this.props.values })
  }

  componentDidMount () {
    if (this.state.init) {
      this.props.getLoDatabyUser()
  ...
  onSelect = (name, value) => {
    ...
    const updatedCapturedData = this.state.capturedLOData || {}
    updatedCapturedData[name] = value
    this.setState({ capturedLOData: updatedCapturedData })
  }
  state = {
    init: true,
    choiceOptions  : [
     ...
     content: (
       <Module7Form
         dataCaptured={this.state ? this.state.capturedLOData : {}}
       />
     )
   },
   {
              ...
              <Col sm='6'>
                ...
                <FormGroup  style={{zIndex:"100" , width:"90%" , position:"relative" , left:'10px'}} className='has-icon-left position-relative'>
                  ...
                  onChange={this.handleChange}
                    ...
                    defaultValue={contraventionOptions[4]}
                    name='clear'                    
              ...
              <Col sm='6' >
                ...
                <FormGroup style={{zIndex:"100"  , width:"90%" , position:"relative" , left:'60px'}} className='has-icon-left position-relative'>
                  ...
                  isMulti
                    className='React'
                    classNamePrefix='select'
                    // defaultValue={contraventionSubOptions[4]}
                    name='clear'
               ...
               <CardBody>
                ...
                <Wizard
                  validate
                  enableAllSteps
                  // onFinish={() => this.props.updateLoDatabyUser(this.state.capturedLOData)
                  // }
                
                  update={() =>
                    this.props.updateLoDatabyUser(this.state.capturedLOData)
                  }
                  steps={steps}
                  nextData={capturedData =>{
                    this.setState({
                      capturedLOData: {...capturedData, ...this.state.capturedLOData}
                    })
                  }}
                  capturedLOData={this.state.capturedLOData}
                />
              </CardBody>{' '}
            </Card>{' '}
          </Col>{' '}
...
const mapStateToProps = state => {
  return {
    values: state.coc ? state.coc.lo.loValues : state.coc
 }
}
