...
import { getBoDatabyUser, updateBoDatabyUser} from '../../../../redux/actions/coc/boAction'
...
class Tabs extends React.Component {
  ...
  componentDidUpdate (prevProps, nextProps) {
    if (prevProps.values !== this.props.values)
      this.setState({ capturedBOData: this.props.values })
  }

  componentDidMount () {
    if (this.state.init) {
      this.props.getBoDatabyUser()
  ...
  onSelect = (name, value) => {
    ...
    const updatedCapturedData = this.state.capturedBOData || {}
    updatedCapturedData[name] = value
    this.setState({ capturedBOData: updatedCapturedData })
  }
  ...
  state = {
    ...
    choiceOptions  : [
     ...
     content: (
       <Module7Form
         dataCaptured={this.state ? this.state.capturedBOData : {}}
       />
     )
   },
   {
              ...
              <Col sm='6'>
                ...
                <FormGroup  style={{zIndex:"100" , width:"90%" , position:"relative" , left:'10px'}} className='has-icon-left position-relative'>
                  ...
                  onChange={this.handleChange}
                    ...
                    defaultValue={contraventionOptions[3]}
                    name='clear'                    
              ...
              <Col sm='6' >
               ...
               <CardBody>
                ...
                <Wizard
                  validate={true}
                  enableAllSteps
                  // onFinish={() => this.props.updateBoDatabyUser(this.state.capturedBOData)
                  // }
            
                  update={() =>
                    this.props.updateBoDatabyUser(this.state.capturedBOData)
                  }
                  
                  nextData={capturedData =>{
                    this.setState({
                      capturedBOData: {...capturedData, ...this.state.capturedBOData}
                    })
                  }}
                  steps={steps}
                  capturedBOData={this.state.capturedBOData}
...
const mapStateToProps = state => {
  return {
    values: state.coc ? state.coc.bo.boValues : state.coc
 }
}
