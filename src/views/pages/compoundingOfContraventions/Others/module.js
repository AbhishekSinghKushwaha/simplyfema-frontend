...
import { getOthersDatabyUser, updateOthersDatabyUser} from '../../../../redux/actions/coc/othersAction'
...
class Tabs extends React.Component {
  ...
  componentDidUpdate (prevProps, nextProps) {
    if (prevProps.values !== this.props.values)
      this.setState({ capturedOthersData: this.props.values })
  }

  componentDidMount () {
    if (this.state.init) {
      this.props.getOthersDatabyUser()
  ...
  onSelect = (name, value) => {
    ...
    const updatedCapturedData = this.state.capturedOthersData || {}
    updatedCapturedData[name] = value
    this.setState({ capturedOthersData: updatedCapturedData })
  }
  state = {
    init: true,
    choiceOptions  : [
     ...
     content: (
       <Module7Form
         dataCaptured={this.state ? this.state.capturedOthersData : {}}
       />
     )
   },
   {
              ...
              <Col sm='6' >
               ...
               <CardBody>
                ...
                <Wizard
                  validate
                  enableAllSteps
                  // onFinish={() => this.props.updateOthersDatabyUser(this.state.capturedOthersData)
                  // }
                
                  update={() =>
                    this.props.updateOthersDatabyUser(this.state.capturedOthersData)
                  }
                  steps={steps}
                  nextData={capturedData =>{
                    this.setState({
                      capturedOthersData: {...capturedData, ...this.state.capturedOthersData}
                    })
                  }}
                  capturedOthersData={this.state.capturedOthersData}
                />
              </CardBody>{' '}
            </Card>{' '}
          </Col>{' '}
...
const mapStateToProps = state => {
  return {
    values: state.coc ? state.coc.others.othersValues : state.coc
 }
}
