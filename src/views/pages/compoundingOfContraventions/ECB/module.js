...
import { getEcbDatabyUser, updateEcbDatabyUser} from '../../../../redux/actions/coc/ecbAction'
...
class Tabs extends React.Component {
  ...
  componentDidUpdate (prevProps, nextProps) {
    if (prevProps.values !== this.props.values)
      this.setState({ capturedECBData: this.props.values })
  }

  componentDidMount () {
    if (this.state.init) {
      this.props.getEcbDatabyUser()
  ...
  onSelect = (name, value) => {
    ...
    const updatedCapturedData = this.state.capturedECBData || {}
    updatedCapturedData[name] = value
    this.setState({ capturedECBData: updatedCapturedData })
  }
  ...
  state = {
    ...
    choiceOptions  : [
     ...
     content: (
       <Module7Form
         dataCaptured={this.state ? this.state.capturedECBData : {}}
       />
     )
   },
   {
              ...
              <Col sm='6'>
                ...
                <FormGroup  style={{zIndex:"100" , width:"90%" , position:"relative" , left:'10px'}} className='has-icon-left position-relative'>
                  ...
                  onChange={this.handleChange}
                    ...
                    defaultValue={contraventionOptions[2]}
                    name='clear'                    
              ...
              <Col sm='6' >
               ...
               <CardBody>
                ...
                <Wizard
                  validate
                  enableAllSteps
                  // onFinish={() => this.props.updateEcbDatabyUser(this.state.capturedECBData)
                  // }
                
                  update={() =>
                    this.props.updateEcbDatabyUser(this.state.capturedECBData)
                  }
                  steps={steps}
                  nextData={capturedData =>{
                    this.setState({
                      capturedECBData: {...capturedData, ...this.state.capturedECBData}
                    })
                  }}
                  capturedECBData={this.state.capturedECBData}
                />
              </CardBody>{' '}
            </Card>{' '}
          </Col>{' '}
...
const mapStateToProps = state => {
  return {
    values: state.coc ? state.coc.ecb.ecbValues : state.coc
 }
}
