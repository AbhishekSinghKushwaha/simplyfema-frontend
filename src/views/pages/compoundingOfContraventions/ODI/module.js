...
import { getOdiDatabyUser, updateOdiDatabyUser} from '../../../../redux/actions/coc/odiAction'
...
class Tabs extends React.Component {
  ...
  componentDidUpdate (prevProps, nextProps) {
    if (prevProps.values !== this.props.values)
      this.setState({ capturedODIData: this.props.values })
  }

  componentDidMount () {
    if (this.state.init) {
      this.props.getOdiDatabyUser()
  ...
  onSelect = (name, value) => {
    ...
    const updatedCapturedData = this.state.capturedODIData || {}
    updatedCapturedData[name] = value
    this.setState({ capturedODIData: updatedCapturedData })
  }
              ...
              <Col sm='6'>
                ...
                <FormGroup  style={{zIndex:"100" , width:"90%" , position:"relative" , left:'10px'}} className='has-icon-left position-relative'>
                  ...
                  onChange={this.handleChange}
                    ...
                    defaultValue={contraventionOptions[1]}
                    name='clear'                    
              ...
              <Col sm='6' >
               ...
               <CardBody>
                ...
                <Wizard
                  validate
                  enableAllSteps
                  // onFinish={() => this.props.updateOdiDatabyUser(this.state.capturedODIData)
                  // }
                
                  update={() =>
                    this.props.updateOdiDatabyUser(this.state.capturedODIData)
                  }
                  steps={steps}
                  nextData={capturedData =>{
                    this.setState({
                      capturedODIData: {...capturedData, ...this.state.capturedODIData}
                    })
                  }}
                  capturedODIData={this.state.capturedODIData}
                />
              </CardBody>{' '}
            </Card>{' '}
          </Col>{' '}
...
const mapStateToProps = state => {
  return {
    values: state.coc ? state.coc.odi.odiValues : state.coc
 }
}
