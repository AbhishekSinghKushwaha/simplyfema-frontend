...
import { getPoDatabyUser, updatePoDatabyUser} from '../../../../redux/actions/coc/poAction'
...
class Tabs extends React.Component {
  ...
  componentDidUpdate (prevProps, nextProps) {
    if (prevProps.values !== this.props.values)
      this.setState({ capturedPOData: this.props.values })
  }

  componentDidMount () {
    if (this.state.init) {
      this.props.getPoDatabyUser()
  ...
  onSelect = (name, value) => {
    ...
    const updatedCapturedData = this.state.capturedPOData || {}
    updatedCapturedData[name] = value
    this.setState({ capturedPOData: updatedCapturedData })
  }
  state = {
    init: true,
    choiceOptions  : [
     ...
     content: (
       <Module7Form
         dataCaptured={this.state ? this.state.capturedPOData : {}}
       />
     )
   },
   {
              ...
              <Col sm='6'>
                ...
                <FormGroup  style={{zIndex:"100" , width:"90%" , position:"relative" , left:'10px'}} className='has-icon-left position-relative'>
                  ...
                  onChange={this.handleChange}
                    ...
                    defaultValue={contraventionOptions[5]}
                    name='clear'                    
              ...
              <Col sm='6' >
               ...
               <CardBody>
                ...
                <Wizard
                  validate
                  enableAllSteps
                  onFinish={() => this.props.updatePoDatabyUser(this.state.capturedPOData)
                  }
                
                  update={() =>
                    this.props.updatePoDatabyUser(this.state.capturedPOData)
                  }
                  steps={steps}
                  nextData={capturedData =>{
                    this.setState({
                      capturedPOData: {...capturedData, ...this.state.capturedPOData}
                    })
                  }}
                  capturedPOData={this.state.capturedPOData}
                />
              </CardBody>{' '}
            </Card>{' '}
          </Col>{' '}
...
const mapStateToProps = state => {
  return {
    values: state.coc ? state.coc.po.poValues : state.coc
 }
}
