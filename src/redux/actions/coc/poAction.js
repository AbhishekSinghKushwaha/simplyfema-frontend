...
export const getPoDatabyUser = user => {
    return dispatch => {
      dispatch({
        type: 'GET_PO_DATA_BY_USER_LOADING'
      })
      
      
      axios.get('http://65.1.215.55:4003/coc/po/alldata/e44fdeef-922e-4c2f-9f0c-107ae1f1cc5555')
      .then(response => {
        var fetchPoUserData
        if (response.data) {
        fetchPoUserData = response.data.entityData
        fetchPoUserData = Object.assign({}, ...fetchPoUserData)
        if (response.err) {
          console.log(response.err)
        }

        dispatch({
            type: 'GET_PO_DATA_BY_USER_SUCCESS',
            payload: {
              data: fetchPoUserData
            }
          })
      }
  })
      .catch(err => {
        dispatch({
          type: 'GET_PO_DATA_BY_USER_FAILURE'
        })
        console.log(err)
      })
  }
  ...
  export const updatePoDatabyUser = entityData => {
    return dispatch => {
      dispatch({
    type: 'UPDATE_PO_DATA_BY_USER_LOADING'
  })
  ...
  let userId =
      ...
      axios.post(`http://localhost:4001/coc/po/alldata/${userId}`,
          {
            poAllDetails: {
              entityDetails: {
                //"entityCode":149,
              ...
              poDetails:{
                ...
                dateIncorporationpo:entityData.dateIncorporationpo,
    ...
    .then(response => {
      var updatedPoUserData

      if(response.data){
        updatedPoUserData = response.data

        dispatch({
          type: 'UPDATE_PO_DATA_SUCCESS',
          payload: {
            updatedPoUserData
          }
        })
    ...
    .catch(err => {
      dispatch({
        type: 'UPDATE_PO_DATA_BY_USER_FAILURE'
      })
      console.log(err)
    })
    }
