...
export const getOdiDatabyUser = user => {
    return dispatch => {
      dispatch({
        type: 'GET_ODI_DATA_BY_USER_LOADING'
      })
      
      
      axios.get('http://65.1.215.55:4003/coc/odi/alldata/e44fdeef-922e-4c2f-9f0c-107ae1f1cc5555')
      .then(response => {
        var fetchOdiUserData
        if (response.data) {
        fetchOdiUserData = response.data.entityData
        fetchOdiUserData = Object.assign({}, ...fetchOdiUserData)
        if (response.err) {
          console.log(response.err)
        }

        dispatch({
            type: 'GET_ODI_DATA_BY_USER_SUCCESS',
            payload: {
              data: fetchOdiUserData
            }
          })
      }
  })
      .catch(err => {
        dispatch({
          type: 'GET_ODI_DATA_BY_USER_FAILURE'
        })
        console.log(err)
      })
  }
  ...
  export const updateOdiDatabyUser = entityData => {
    return dispatch => {
      dispatch({
    type: 'UPDATE_ODI_DATA_BY_USER_LOADING'
  })
  ...
  let userId =
      ...
      axios.post(`http://localhost:4001/coc/odi/alldata/${userId}`,
          {
            odiAllDetails: {
              entityDetails: {
                //"entityCode":149,
              ...
              odiDetails:{
                //"odiCode":154854,
                nameTheApplicant:entityData.nameOfTheApplicant,
                dateIncorporationodi:entityData.dateIncorporationodi,
                incometaxPan:entityData.incomeTaxPan,
                natureActivitiesUnderTaken:entityData.natureOfActivitiesUnderTaken,
                nameOverseasEntity:entityData.nameOfOverseasEntity,
                dOIOverseasEntity:entityData.dOIofOverseasEntity,
                natureActivitiesUnderTakenByOverseasEntity:entityData.natureOfActivitiesUnderTakenByOverseasEntity,
                natureEntityWOSorJV:entityData.natureOfEntityWOSorJV,
                detailsRemittanceSentandDateOfRemittanceandAmountInFCYandInINR:entityData.detailsOfRemittanceSentandDateOfRemittanceandAmountInFCYandInINR,
                detailsOtherFinancialCommitment:entityData.detailsOfOtherFinancialCommitment,
                detailsUINAppliedAndRecieved:entityData.detailsOfUINAppliedAndRecieved,
                dateReciept:entityData.dateReciept,
                approvalOtherRegulatorsIfRequired:entityData.approvalOfOtherRegulatorsIfRequired,
                detailsAPRSubmitted:entityData.detailsOfAPRSubmitted,
                natureContraventionsAndReasonForTheContravention:entityData.natureOfContraventionsAndReasonForTheContravention,
    ...
    .then(response => {
      var updatedOdiUserData

      if(response.data){
        updatedOdiUserData = response.data

        dispatch({
          type: 'UPDATE_ODI_DATA_SUCCESS',
          payload: {
            updatedOdiUserData
          }
        })
    ...
    .catch(err => {
      dispatch({
        type: 'UPDATE_ODI_DATA_BY_USER_FAILURE'
      })
      console.log(err)
    })
    }
