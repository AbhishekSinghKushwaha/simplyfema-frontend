...
class Autocomplete extends React.Component {
  ...
  renderUngroupedSuggestions = () => {
    const { filterKey,subfilterKey, suggestions, customRender, suggestionLimit } = this.props
    const {
      onSuggestionItemClick,
      onSuggestionItemHover,
      state: { activeSuggestion, userInput }
    ...
    let sortSingleData = suggestions
      .filter(i => {
        let startCondition = i[subfilterKey]
            .toLowerCase()
            .startsWith(userInput.toLowerCase()),
          includeCondition = i[subfilterKey]
            .toLowerCase()
            .includes(userInput.toLowerCase()),
          regexCondition = i[subfilterKey]
          .toLowerCase()
          .test('/'+ userInput.toLowerCase()+'/')
        if (startCondition) {
        ...
        else if (!startCondition && !includeCondition && regexCondition){
          return regexCondition
        }
        else {
          return null
  ...
  // Renders Suggestions
  renderSuggestions = () => {
    const { filterKey, subfilterKey,grouped, filterHeaderKey, suggestions } = this.props
    const {
      renderUngroupedSuggestions,
      state: { userInput }
    } = this
...
Autocomplete.propTypes = {
  ...
  subfilterKey : PropTypes.string.isRequired,
